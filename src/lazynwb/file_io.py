...
class FileAccessor:
    ...
    def __init__(
        ...
        if isinstance(path_or_accessor, (h5py.File, h5py.Group, zarr.Group)):
            ...
            self._path = npc_io.from_pathlike(path_or_accessor)
    ...
    def get_hdmf_backend(self) -> HDMFBackend:
        if isinstance(self._accessor, (h5py.File, h5py.Group)):
            return HDMFBackend.HDF5
        elif isinstance(self._accessor, zarr.Group):
            return HDMFBackend.ZARR
...
class Safe:
    """
    Class containing decorators for safely working with files. The decorators automatically
    handle opening and closing files for input arguments that are file paths.
    """
    ...
    def auto_file_close(
        ...
        @Safe.auto_file_close()  # Uses default opener from file_io
        ...
        @Safe.auto_file_close(h5py.File)  # Specify a specific opener
        def another_func(file1, file2, other_arg=None):
            # file1 and file2 will be automatically opened with h5py.File and closed
            ...

        @Safe.auto_file_close(use_thread_pool=True)  # Use parallel opening
        ...
        Args:
            open_func: Function to use for opening files if a path is provided.
                    Defaults to the open function from file_io
                ...
                with ExitStack() as stack:
                    ...
                    if file_paths_to_open:
                        if use_thread_pool and (
                            ...
                            or any(
                                item.get("is_collection") for item in file_paths_to_open
                            )
                        ):
                            ...
                            for item in file_paths_to_open:
                                if not item["is_collection"]:
                                    ...
                                    # Wrap in FileAccessor for consistent behavior
                                    new_kwargs[item["param_name"]] = (
                                        stack.enter_context(FileAccessor(accessor))
                                    )
                                else:
...
# Add a top-level alias for convenience
auto_file_close = Safe.auto_file_close
